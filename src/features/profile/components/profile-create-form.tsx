...
const ProfileCreateForm: React.FC<ProfileFormType> = ({
  ...
  const title = initialData ? '' : 'Atualize o seu perfil';
  const description = initialData
    ? 'Edit a product.'
    : 'Preencha os próximos campos para atualizar o seu perfil';
  const toastMessage = initialData ? 'Product updated.' : 'Product created.';
  const action = initialData ? 'Guardar' : 'Criar';
  ...
  const onSubmit = async (data: ProfileFormValues) => {
    try {
      ...
      router.refresh();
      router.push(`/dashboard/metricss`);
    } catch (error: any) {
    } finally {
      setLoading(false);
    }
